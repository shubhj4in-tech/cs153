...
# ─────────────────────────────────────────────────────────────────────────────
# Warm-init hook: write a PYTHONSTARTUP script that monkey-patches
# Deformable-3DGS to load pre-trained deformation MLP weights.
# ─────────────────────────────────────────────────────────────────────────────

def _write_warm_init_hook(warm_weights_path: Path, output_dir: Path) -> None:
    """
    Write a PYTHONSTARTUP script that intercepts the Deformable-3DGS deformation
    model creation and loads warm weights before training begins.

    The script is placed in the output dir and referenced via PYTHONSTARTUP env var.
    """
    hook_path = output_dir / "_warm_init_hook.py"
    hook_code = f"""
# Auto-generated by 03_reconstruct.py — warm-init hook for Deformable-3DGS.
import importlib, torch, sys, os

_WARM_PATH = {str(warm_weights_path)!r}
_warm_state = torch.load(_WARM_PATH, map_location="cpu")

# Monkey-patch: after any module named 'deformation' (or similar) is imported
# and a deformation model is created, load the warm state dict.
_original_import = __builtins__.__import__ if hasattr(__builtins__, "__import__") else __import__

def _patched_import(name, *args, **kwargs):
    mod = _original_import(name, *args, **kwargs)
    if "deform" in name.lower():
        # Patch class constructors to load warm weights post-init
        for attr_name in dir(mod):
            attr = getattr(mod, attr_name, None)
            if isinstance(attr, type) and "Deform" in attr_name:
                _orig_init = attr.__init__
                def _new_init(self, *a, __cls=attr, __orig=_orig_init, **kw):
                    __orig(self, *a, **kw)
                    try:
                        self.load_state_dict(_warm_state, strict=False)
                        print(f"[WarmInit] Loaded warm weights into {{__cls.__name__}}")
                    except Exception as e:
                        print(f"[WarmInit] Could not load warm weights: {{e}}")
                attr.__init__ = _new_init
    return mod
...
"""
    ...
    os.environ["PYTHONSTARTUP"] = str(hook_path)
    print(f"[Stage 3] Warm-init hook written → {hook_path}")
...
def _do_reconstruct_foreground(scene_dir: Path, warm_init_path: str | None,
    ...
    if warm_init_path:
        # Deformable-3DGS has no --warm_init_path flag, so we inject the weights
        # by writing a small init-hook script that the training process will
        # source via PYTHONSTARTUP before importing the deformation module.
        _write_warm_init_hook(Path(warm_init_path), output_dir)
